...
async function createProject(name: string): Promise<any> {
  const res = await SELFfetch("http://x/api/projects", {
    method: "POST",
...
describe("projects API", () => {
  it("creates, lists, summarises and deletes a project", async () => {
    ...
    const list = (await (await SELFfetch("http://x/api/projects")).json()) as any[];
    expect(list.some((p) => p.id === project.id)).toBe(true);

    const summary = (await (await SELFfetch(`http://x/api/projects/${project.id}/summary`)).json()) as any;
    expect(summary.project.id).toBe(project.id);
    expect(summary.counts.users).toBe(0);
    expect(summary.webex).toBeNull();

    const del = await SELFfetch(`http://x/api/projects/${project.id}`, { method: "DELETE" });
    expect(del.status).toBe(200);
    const after = await SELFfetch(`http://x/api/projects/${project.id}`);
...
describe("crypto roundtrip via AXL config", () => {
  it("stores an AXL connection and reports it in the summary", async () => {
    ...
    const save = await SELFfetch(`http://x/api/projects/${project.id}/axl`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
